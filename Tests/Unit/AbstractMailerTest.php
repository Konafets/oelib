...
class Tx_Oelib_AbstractMailerTest extends Tx_Phpunit_TestCase {
	...
	/**
	 * Returns the first sent-email or NULL if non has been sent.
	 *
	 * @return t3lib_mail_Message|NULL
	 */
	protected function getFirstSendEmail() {
		$sendEmails = $this->subject->getSentEmails();

		return empty($sendEmails) ? NULL : $sendEmails[0];
	}
	...
	public function sendSetsSenderNameAndEmail() {
		...
		$sentEmail = $this->getFirstSendEmail();
		$this->assertSame(
	...
	public function sendForTwoRecipientsSendsTwoEmails() {
		...
		$sentEmails = $this->subject->getSentEmails();
		$this->assertSame(
			2,
			count($sentEmails)
		);
	...
	public function sendingPlainTextMailUsesDefaultCharacterSet() {
		...
		$this->assertSame(
			...
			$this->getFirstSendEmail()->getCharset()
		);
	}
	...
	public function sendingPlainTextMailUsesPlainTextEncoding() {
		...
		$this->assertSame(
			'text/plain',
			$this->getFirstSendEmail()->getContentType()
		);
	}
	...
	public function sendingPlainTextMailByDefaultRemovesAnyCarriageReturnFromBody() {
		...
		$this->assertNotContains(
			CR,
			$this->getFirstSendEmail()->getBody()
		);
	}
	...
	public function sendSetsHtmlBody() {
		...
		$children = $this->getFirstSendEmail()->getChildren();
