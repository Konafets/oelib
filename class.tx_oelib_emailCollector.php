...
class tx_oelib_emailCollector extends tx_oelib_abstractMailer {
	 ...
	 * first dimension depends on how many e-mails are currently stored.
	 * One stored e-mail is always an associative array with three elements
	 * named 'recipient', 'subject' and 'message'.
	 */
	 ...
	 *
	 * @return	array		e-mail address, subject and message of the last e-mail
	 * 						in an array, will be empty if there is no e-mail
	 */
	 ...
	 * none.
	 *
	 * @return	array		two-dimensional array with one element for each e-mail,
	 * 						each inner array has three the elements 'recipient',
	 * 						'subject' and 'message', will be empty if there are
	 * 						no e-mails
	 *
	 * @see		emailData
	 */
	...
	private function getElementFromLastEmail($key) {
		if (!in_array($key, array('recipient', 'subject', 'message'))) {
			throw new Exception(
				'The key "'.$key.'" is invalid. '
					.'It must be "recipient", "subject" or "message".'
			);
		}
		if (empty($this->emailData)) {
			return '';
